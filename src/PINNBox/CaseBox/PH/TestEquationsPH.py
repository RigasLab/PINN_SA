...
def RANS_B_RSTE(rho,nu,fp):
    ...
    return pde
#################################################################
def RANS_B_RSTE_varFP(rho,nu,fp):
    mu = nu*rho
    def pde(X,Q,D):
        U       = Q[:,0:1]
        V       = Q[:,1:2]
        ...
        dAx_u = tf.reshape(A[0:1000,0:1],tf.shape(P_u))
        dAy_u = tf.reshape(A[0:1000,1:2],tf.shape(P_u))

        dAx_l = tf.reshape(A[1000:2000,2:3],tf.shape(P_l))
        dAy_l = tf.reshape(A[1000:2000,3:4],tf.shape(P_l))

        tau11 = 2*mu*Ux
        tau12 = mu*(Uy+Vx)

        tau11_u = tau11[0:1000,0:1]
        tau11_l = tau11[1000:2000,0:1]

        tau12_u = tau12[0:1000,0:1]
        tau12_l = tau12[1000:2000,0:1]

        dF1 = -(P_u*dAy_u + P_l*dAy_l) + (tau11_u*dAy_u + tau11_l*dAy_l) + (tau12_u*dAx_u + tau12_l*dAx_l)

        fp = -tf.math.reduce_sum(dF1)/vol
        #####################################################
        #####################################################
        cont  = Ux + Vy
        xmom  = U*Ux + V*Uy + (1/rho)*Px - nu*(Uxx + Uyy) + uux + uvy + fp
...
def RANS_B_HD(rho,nu,fp):
    ...
    return pde
#################################################################
def RANS_B_HD_varFP(rho,nu,vol):
    mu = nu*rho
    def pde(X,Q,D):
        U       = Q[:,0:1]
        V       = Q[:,1:2]
...
def RANS_SA_HD(rho,nu,minS,fp):
    ...
    return pde
#################################################################
def RANS_SA_HD_varFP(rho,nu,minS,vol):
    mu = nu*rho
